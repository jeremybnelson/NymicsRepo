...
class Option:
	...
	def __init__(self, environ_var='', options=''):
		...
		# track final option-value sources for diagnostics
		self.command_line = str(sys.argv[1:])
		self.environ_var = environ_var
		self.options = options.strip()
		self.option_source = dict()

		# lowest priority: code injected options (--option=value ...)
		context = f'program injection ({self.options})'
		for option in shlex.split(options):
			self.set_option(option, context)

		# medium priority: env var option (udp_<script> = --option=value ...)
		self.environ_options = os.getenv(environ_var, '').strip()
		context = f'environment variable ({self.environ_var}={self.environ_options})'
		for option in shlex.split(self.environ_options):
			self.set_option(option, context)

		# highest priority: command line settings (--option=value ...)
		context = f'command line ({self.command_line})'
