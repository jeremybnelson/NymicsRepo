...
Note: OSError replaces IOError, EnvironmentError, WindowsError, VMSError, socket.error, select.error and mmap.error.
https://stackoverflow.com/questions/29347790/difference-between-ioerror-and-oserro

###

TODO: move code from wip/udp_start.py to log_session_info.
TODO: Review wip/ for similar session/env info to refactor in as well.

TODO: psutil additions
- machine total memory
- process CPU that matches Task Manager
- process memory usage (used, %)
...
This ensures your code and dependencies are isolated from the system Python installation

###

Future:
- Unittest, pytest, and/or doctest test harnesses
- Coverage harness
- Profiling harness
...
# TODO: Optional parameter to convert to UTC/GMT or other timezone.
def now():
...
def is_folder(path_name):
	...
	return pathlib.Path(path_name).is_dir()
...
def hash_file(file_name, hash_method_name=None):
	...
	with open(file_name, 'rb') as input_stream:
		hash_value = hash_method(input_stream.read()).hexdigest()
	return hash_value
...
def log_setup(log_file_name=None, log_level=logging.INFO):
	...
	log_base_format = '%(filename)s:%(lineno)d | %(funcName)s | %(levelname)s | %(message)s'
	if log_file_name:
		log_format = f'%(asctime)s | {log_base_format}'
		logging.basicConfig(filename=log_file_name, format=log_format, datefmt=log_timestamp, level=log_level)
	else:
		log_format = f'LOG: {log_base_format}'
		logging.basicConfig(stream=sys.stdout, format=log_format, datefmt=log_timestamp, level=log_level)
...
def parent_path(path_name):
	...
	path = pathlib.Path(path_name)
	if path.is_folder():
		# return folder's parent path
		return path.parent
	else:
		# return file's folder path's parent path
		return path.parent
...
"""
RSS used for process specific memory metrics to maintain compatiblity with Task Manager metrics.
...
def script_name():
	...
	return just_file_stem(sys.argv[0])
...
def load_text(file_name, default=None, encoding='UTF8'):
	...
	# return default value if exception during file open
		...
		if is_file(file_name):
			logger.warning(f'Unable to open ({file_name}); error: ({e}); returning default ({default})')
		else:
			logger.debug(f'File does not exist ({file_name}); returning default ({default})')
		text = default
...
def make_fdqn(name):
	"""Convert name into a FDQN compatible format, eg. lowercase, hyphen vs underscores."""
...
def split(items, delimiters=', ', strip=True):
	...
	if isinstance(items, str):
		...
		# split on normalized delimiter
		items = items.split(normalized_delimiter)

		# optionally strip split items
		if strip:
...
class FileList:
	...
	def file_names(self):
		"""Return sorted, deduped list of file names built through multiple include/exclude steps."""
...
def _archive(archive_file_name, file_names, relative_path='', mode=''):
	...
	relative_path = path_with_separator(relative_path)

	with zipfile.ZipFile(archive_file_name, mode, compression=zipfile.ZIP_DEFLATED) as archive_file:
...
def extract_archive(archive_file_name, target_folder, file_names):
	...
	# insure paths end with '/'
	target_folder = path_with_separator(target_folder)
...
def test_archive():
	...
	# test long paths
	file_list = FileList()
	# source_folder_name = '../test2/evercore'
	# source_folder_name = '/users/malcolmgreene/test2/evercore'
...
def test_make_key():
	"""Test make_key()."""
	logger.info(f'make_key(A,b,c, delimiter="=") = {make_key("A", "b", "c", delimiter="-")}')
...
def test_string_cleanup():
	...
	logger.info(f'After left_trim(case_sensitive=False): {left_trim(text, "CAT", case_sensitive=False)}')
	logger.info(f'After right_trim(case_sensitive=False): {right_trim(text, "cat", case_sensitive=False)}')
	logger.info(f'After all_trim(case_sensitive=False): {all_trim(text, "CaT", case_sensitive=False)}')
