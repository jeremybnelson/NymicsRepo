...
class Config:
	...
	def __init__(self, *path_names):
		...
		self.comment_chars = ('#', ';')
		self.secret_substrings = 'cert password private pwd secret secure ssh'.split()
...
class ConfigSectionKey(Config):
	...
	def debug(self, key, value, context=''):
		...
		if not self.current_section._is_validated:
			if context:
		...
		if context:
			logger.info(f'{self.file_name}({self.line_number}): [{self.current_section_key}].{key} = {value}; {context}')
		else:
			logger.info(f'{self.file_name}({self.line_number}): [{self.current_section_key}].{key} = {value}')
	...
	def load_file(self, file_name, default_section_key=''):
		...
		for self.line_number, line in enumerate(lines, 1):
			...
			# process key=value assignments
				...
				elif self.current_section._is_validated and not hasattr(self.current_section, key):
					self.warning(f'Unknown property ({key})')
				else:
					# update the value
	...
	def load_section(self, line):
		...
		if section_key in self.sections:
			...
			logger.info(f'{self.file_name}({self.line_number}): [{section_key}] @update')
			self.current_section = self.sections[section_key]

		# create a new section object or set to None if section_key is invalid
		else:
			self.is_section_new = True
			logger.info(f'{self.file_name}({self.line_number}): [{section_key}]')
	...
	def set_key_value(self, key, key_id, value, context=''):
		...
		# Future: Track previous value for diagnostic purposes.
